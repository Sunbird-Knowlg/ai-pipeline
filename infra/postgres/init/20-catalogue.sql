...
--
-- Applied when Postgres is provisioned, not by the application. In compose that is
-- `/docker-entrypoint-initdb.d`, which the official image runs once against an empty data directory;
-- in a real environment it is whatever provisions the database (a Terraform/RDS bootstrap, a
-- platform job, a DBA). The API assumes these tables already exist and never creates them, so it
-- needs no DDL privileges at run time.
--
-- Written to be re-runnable (`IF NOT EXISTS` throughout), so applying it to an existing database is
-- safe. Note that `docker-entrypoint-initdb.d` only runs on *first* initialisation: after a
-- `docker compose down` without `-v`, an existing volume is not re-provisioned. Changing this file
-- therefore needs either `down -v` locally, or applying the change by hand:
