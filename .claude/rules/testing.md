...
make test-integration   # same, -m integration; requires `make dev-up` first (Kafka, Azurite)
...
- **Mock the seams, don't hit real infrastructure in unit tests**: mock `BlobStorageUtil` and
  `KnowlgClient` (or the specific methods used) rather than connecting to a real blob
  backend/HTTP endpoint. Reserve real infrastructure for tests marked `integration`, which
  assume `make dev-up` has already been run.
- **Cover normal, edge, and failure paths** — in particular `caption-generator`'s DLQ path (an
  exception during processing should mark the Transcript node `Failed` via `knowlg.patch(...)`
  and emit to the DLQ side output — see `kafka-contracts.md`), and the "skip, don't process"
  early-return branches in `content_published.py`/`transcript_approved.py`.
