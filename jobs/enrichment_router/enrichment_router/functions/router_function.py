    ...
    def process_element(self, value: str, ctx):
        ...
        try:
            if event.contentType == "Content":
                self._handle_content_published(event, ctx)
            elif event.contentType == "Transcript" and event.action == "approved":
                self._handle_transcript_approved(event, ctx)
        except Exception as error:
            self.logger.exception("Failed routing event %s: %s", event.id, error)

    def _handle_content_published(self, event: EnrichedMetadataEvent, ctx) -> None:
        ...
        if request is not None:
            ctx.output(TRANSCRIPTION_OUT_TAG, request.to_json())

    def _handle_transcript_approved(self, event: EnrichedMetadataEvent, ctx) -> None:
        assert self.graph is not None, "open() must be called before process_element()"
        ...
        if request is not None:
            ctx.output(MULTILINGUAL_OUT_TAG, request.to_json())
