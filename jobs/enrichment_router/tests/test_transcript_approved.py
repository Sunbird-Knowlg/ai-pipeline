...
def test_skips_non_source_language_approval(transcript_approved_event, mock_graph):
    transcript_approved_event.data["sourceLanguage"] = False

    result = handle_transcript_approved(transcript_approved_event, mock_graph, LANGUAGES)

    assert result is None
    mock_graph.get_related_nodes.assert_not_called()


def test_skips_when_no_languages_configured(transcript_approved_event, mock_graph):
    result = handle_transcript_approved(transcript_approved_event, mock_graph, [])

    assert result is None


def test_skips_when_all_languages_already_active(transcript_approved_event, mock_graph):
...
def test_creates_drafts_and_emits_for_new_languages(transcript_approved_event, mock_graph):
    mock_graph.get_related_nodes.return_value = [
        {
...
def test_creates_only_missing_languages(transcript_approved_event, mock_graph):
    mock_graph.get_related_nodes.return_value = [
        {
            "IL_UNIQUE_ID": "do_t_en",
            "languageCode": "en",
...
def test_excludes_source_language_from_targets(transcript_approved_event, mock_graph):
    ...
    result = handle_transcript_approved(transcript_approved_event, mock_graph, ["en", "hi", "ta"])
...
def test_created_transcript_includes_display_language_name(transcript_approved_event, mock_graph):
    mock_graph.get_related_nodes.return_value = [
        {
            "IL_UNIQUE_ID": "do_t_en",
            "languageCode": "en",
    ...
    handle_transcript_approved(transcript_approved_event, mock_graph, ["hi"])
...
def test_wires_relation_from_enrichment_to_new_transcript(transcript_approved_event, mock_graph):
    mock_graph.get_related_nodes.return_value = [
        {
            "IL_UNIQUE_ID": "do_t_en",
            "languageCode": "en",
    ...
    _, transcript_id, _ = mock_graph.create_node.call_args.args
    from_id, to_id, relation_label = mock_graph.create_relation.call_args.args
    assert from_id == "do_enrich_1"
    assert to_id == transcript_id
    assert relation_label == "associatedTo"
